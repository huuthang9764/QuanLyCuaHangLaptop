...
double tinhLoiNhuanChiTietHoaDon(const DanhSachLaptop& dSLT, ChiTietHoaDon& cTHD) {
    Laptop* pLaptop = TimKiemLaptop(dSLT, cTHD.maLaptop);
    if (pLaptop != NULL) {
        return (cTHD.donGia - pLaptop->giaVon) * cTHD.soLuong;
    }
    return 0;
}
...
void XuatBaoCaoDoanhThuNgay(Date ngayCanLoc, const DanhSachHoaDon& g_dsHoaDonToanCuc) {
    ...
    for (NodeHoaDon* k = g_dsHoaDonToanCuc.head; k != NULL; k = k->next) {
        HoaDon hd = k->data;
...
void XuatBaoCaoTonKho(const DanhSachLaptop& g_dsLaptop) {
    ...
    long long tongSL = 0;
    for (NodeLaptop* k = g_dsLaptop.head; k != NULL; k = k->next) {
        Laptop lt = k->data;
...
double TinhLoiNhuanNgay(const DanhSachLaptop& dslt, const DanhSachHoaDon& dshd, Date ngayCanLoc) {
    ...
    for (NodeHoaDon* k = dshd.head; k != NULL; k = k->next) {
        HoaDon hd = k->data;
        if (hd.ngayLap.ngay == ngayCanLoc.ngay && hd.ngayLap.thang == ngayCanLoc.thang && hd.ngayLap.nam == ngayCanLoc.nam) {
            for (NodeChiTiet* ct = hd.dsChiTiet.head; ct != NULL; ct = ct->next) {
                tong += tinhLoiNhuanChiTietHoaDon(dslt, ct->data);
            }
        }
    }
    return tong;
...
void MenuLoiNhuan(const DanhSachHoaDon& dshd, const DanhSachLaptop& dslt) {
    ...
    cout << ">> TONG LOI NHUAN: " << loiNhuan << " VND\n";
}
...
void MenuThongKe(DanhSachHoaDon& dshd, const DanhSachLaptop& dslt) {
    ...
    do {
        ...
        case '1':
            ...
            break;
        case '2':
            XuatBaoCaoTonKho(dslt);
            break;
        case '3':
            MenuLoiNhuan(dshd, dslt);
            break;
        case '0': break;

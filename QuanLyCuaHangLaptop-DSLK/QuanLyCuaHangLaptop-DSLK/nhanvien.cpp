...
// =================== HELPER: IN DANH SÁCH (DÙNG CON TRỎ) ===================
...
void XuatDanhSachNhanVien(const DanhSachNhanVien& dsNV) {
    ...
    for (NodeNhanVien* k = dsNV.head; k != NULL; k = k->next) {
        cout << "| " << left << setw(10) << k->data.maNV
            << "| " << left << setw(25) << k->data.tenNV
            << "| " << left << setw(15) << k->data.chucVu
            << "| " << left << setw(15) << k->data.soDT
            << "| " << left << setw(20) << k->data.email << " |\n";
    }
    cout << "-----------------------------------------------------------------------------------\n";
}
...
void XuatDanhSachTaiKhoan(const DanhSachTaiKhoan& dsTK) {
    ...
    for (NodeTaiKhoan* k = dsTK.head; k != NULL; k = k->next) {
        cout << "| " << left << setw(20) << k->data.tenDangNhap
            << "| " << left << setw(10) << k->data.maNV
            << "| " << left << setw(10) << (k->data.quyen == QUAN_LY ? "Quan Ly" : "Nhan Vien")
            << "| " << left << setw(15) << (k->data.trangThai == TK_HOAT_DONG ? "Hoat Dong" : "DA KHOA") << " |\n";
    }
    cout << "-------------------------------------------------------------\n";
}

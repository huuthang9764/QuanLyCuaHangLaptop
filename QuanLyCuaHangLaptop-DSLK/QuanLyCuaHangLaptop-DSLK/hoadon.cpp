...
void InChiTietGioHang(const HoaDon& hd, DanhSachLaptop& dsLT) {
    ...
    int i = 1;
    // [DSLK]: Duyệt danh sách liên kết Chi Tiết
...
void XuatDanhSachHoaDon(const DanhSachHoaDon& dsHD) {
    ...
    cout << "\n--- DANH SACH HOA DON ---\n";
    // [DSLK]: Duyệt danh sách liên kết Hóa Đơn
...
void LapHoaDonMoi(DanhSachHoaDon& dsHD, DanhSachKhachHang& dsKH, DanhSachLaptop& dsLT, DanhSachBaoHanh& dsBH, TaiKhoan* tkCurrent) {
    ...
    // 1. KHỞI TẠO
    HoaDon hdMoi;
    KhoiTaoDanhSachChiTiet(hdMoi.dsChiTiet); // Khởi tạo head=NULL cho danh sách chi tiết
    hdMoi.tongTien = 0;
    hdMoi.trangThai = 1;
    ...
    do {
        ...
        if (kh == NULL) {
            ...
            if (chonTao == 'y' || chonTao == 'Y') {
                // [CODE ĐÃ FIX]: Dùng cấp phát động và khởi tạo đầy đủ cho DSLK
                KhachHang* khTam = new KhachHang;

                // [QUAN TRỌNG]: Khởi tạo danh sách hóa đơn rỗng cho khách mới
                ...
                if (strlen(khTam->maKH) > 0 && ThemKhachHang(dsKH, *khTam)) {
                    cout << ">> Tao thanh cong!\n";
                    // Với DSLK, khách hàng mới thêm thường nằm ở cuối (Tail)
                    if (dsKH.tail != NULL) {
                        kh = &dsKH.tail->data;
    ...
    while (true) {
        ...
        // Thêm vào danh sách chi tiết (Hàm này đã xử lý việc thêm node cho DSLK)
    ...
    if (xacNhan == 'y' || xacNhan == 'Y') {
        // [DSLK]: Duyệt danh sách chi tiết bằng con trỏ
...
//#include <ctime>
//#include <cstdio>
//#include <cstring> // Dùng cho _stricmp
//
//using namespace std;
//
//// =================== HÀM HỖ TRỢ ===================
...
//            << setw(10) << k->data.maLaptop
//            << setw(20) << (lt ? lt->tenLaptop : "N/A")
//            << setw(15) << k->data.serial // <--- In Serial
//            << setw(15) << DinhDangTien(k->data.thanhTien) << endl;
//    }
//    cout << right << setw(65) << "TONG CONG: " << DinhDangTien(hd.tongTien) << endl;
//}
...
//                if (strlen(khMoi.maKH) > 0 && ThemKhachHang(dsKH, khMoi)) {
//                    cout << ">> Tao thanh cong!\n";
//                    kh = &dsKH.tail->data;
//                    strcpy_s(maKH, kh->maKH);
//                }
//                else cout << "Loi tao moi.\n";
//            }
//        }
//    } while (kh == NULL);
//
...
//        cout << ">> SP: " << lt->tenLaptop << " (Gia: " << DinhDangTien(lt->giaBan) << ")\n";
//
//        // --- NHẬP SERIAL (Thay vì nhập số lượng) ---
//        ChiTietHoaDon ct;
//        strcpy_s(ct.maLaptop, maLT);
//
//        cout << "Nhap So Serial (S/N) cua may: ";
...
//        cin.getline(ct.serial, 20);
//
//        ct.soLuong = 1; // Mặc định 1 máy 1 serial
//        ct.donGia = (long)lt->giaBan;
//        ct.thanhTien = ct.donGia;
//
//        ThemChiTietVaoHoaDon(hdMoi, ct);

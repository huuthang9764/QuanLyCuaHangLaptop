...
void XemLaptopTheoThuongHieu(const DanhSachLaptop& dsls, const char maTH[10]) {
    ...
    cout << "---------------------------------------------------------------------------------------------------\n";
    for (NodeLaptop* k = dsls.head; k != NULL; k = k->next) {
        if (_stricmp(k->data.maThuongHieu, maTH) == 0) {
            XuatThongTinLaptop(k->data);
            found = true;
        }
    }
...
void XemLaptopTheoLoai(const DanhSachLaptop& dsls, const char maLoai[10]) {
    ...
    cout << "---------------------------------------------------------------------------------------------------\n";
    for (NodeLaptop* k = dsls.head; k != NULL; k = k->next) {
        if (_stricmp(k->data.maLoai, maLoai) == 0) {
            XuatThongTinLaptop(k->data);
            found = true;
        }
    }
...
void XemLaptopTheoKhoangGia(const DanhSachLaptop& dsls, long long giaMin, long long giaMax) {
    ...
    cout << "---------------------------------------------------------------------------------------------------\n";
    for (NodeLaptop* k = dsls.head; k != NULL; k = k->next) {
        if (k->data.giaBan >= giaMin && k->data.giaBan <= giaMax) {
            XuatThongTinLaptop(k->data);
            found = true;
        }
    }
...
void XemLaptopTheoCPU(const DanhSachLaptop& dsls) {
    ...
    bool found = false;
    for (NodeLaptop* k = dsls.head; k != NULL; k = k->next) {
        if (strstr(k->data.cauHinh.CPU, cpuCanTim) != NULL) {
            XuatThongTinLaptop(k->data);
            found = true;
        }
    }
...
void XemLaptopTheoRAM(const DanhSachLaptop& dsls) {
    ...
    bool found = false;
    for (NodeLaptop* k = dsls.head; k != NULL; k = k->next) {
        if (k->data.cauHinh.RAM == ramCanTim) {
            XuatThongTinLaptop(k->data);
            found = true;
        }
    }
...
void XemLaptopTheoSSD(const DanhSachLaptop& dsls) {
    ...
    bool found = false;
    for (NodeLaptop* k = dsls.head; k != NULL; k = k->next) {
        if (k->data.cauHinh.SSD == ssdCanTim) {
            XuatThongTinLaptop(k->data);
            found = true;
        }
    }
...
void ChonThuongHieuTuMenu(const DanhSachThuongHieu& dsth, char* maOut) {
    ...
    for (NodeThuongHieu* k = dsth.head; k != NULL; k = k->next) {
        cout << i++ << ". " << k->data.tenThuongHieu
            << " (" << k->data.maThuongHieu << ")" << endl;
    }
    ...
    for (NodeThuongHieu* k = dsth.head; k != NULL; k = k->next) {
        if (i == chon) {
            strcpy_s(maOut, 10, k->data.maThuongHieu);
            cout << ">> Da chon: " << k->data.tenThuongHieu << endl;
            break;
        }
        i++;
    }
...
void ChonLoaiTuMenu(const DanhSachLoai& dsl, char* maOut) {
    ...
    for (NodeLoai* k = dsl.head; k != NULL; k = k->next) {
        if (i == chon) {
            strcpy_s(maOut, 10, k->data.maLoai);
            cout << ">> Da chon: " << k->data.tenLoai << endl;
            break;
        }
        i++;
    }
...
Laptop NhapThongTinLaptop(const DanhSachThuongHieu& dsth, const DanhSachLoai& dsl) {
    ...
    if (dsth.head == NULL || dsl.head == NULL) {
...
void MenuQuanLyLaptop(DanhSachLaptop& dsls, const DanhSachThuongHieu& dsth, const DanhSachLoai& dsl) {
    ...
    do {
        ...
        case '3': {
            ...
            Laptop* found = NULL;
            DoBenchmark(">> Thoi gian Tim de Sua: ", [&]() {
                found = TimKiemLaptop(dsls, ma);
                });
        ...
        case '4': {
            cout << "\n--- 4. Tim kiem Laptop ---\n";
            char ma[10]; cout << "Nhap ma can tim: "; cin.getline(ma, 10);
            Laptop* lt = nullptr;
        ...
        case '6': {
            ...
            cout << "3. Sap xep theo Gia Ban (Tang dan)\n";
            cout << "Chon: "; subChon = _getch();

            if (subChon == '1') {
                DoBenchmark(">> Thoi gian Sort (Ten): ", [&]() {
                    SapXepLaptop_TheoTen(dsls);
                    });
                cout << "\n>> Da sap xep theo Ten!\n";
            }
            else if (subChon == '2') {
                DoBenchmark(">> Thoi gian Sort (ID): ", [&]() {
                    SapXepLaptop_TheoID(dsls);
                    });
                cout << "\n>> Da sap xep theo Gia!\n";
            }
            else if (subChon == '3') {
                DoBenchmark(">> Thoi gian Sort (Gia): ", [&]() {
        ...
        case '8': {
            ...
            DoBenchmark(">> Thoi gian Loc (Hang): ", [&]() {
        ...
        case '9': {
            ...
            DoBenchmark(">> Thoi gian Loc (Loai): ", [&]() {
